...
def load_object_interaction_npz(file_path: str) -> tuple[np.ndarray, np.ndarray, float]:
    """Load packaged object-interaction motion for Holosoma (two-foot / custom NPZ).

    Expected keys (see ``retargeting`` holosoma export CLI):
        - ``global_joint_positions``: (T, 52, 3) float, joint order matching Holosoma ``SMPLH_DEMO_JOINTS``
        - ``object_poses``: (T, 7) float, ``[qw, qx, qy, qz, x, y, z]`` per frame (same convention as InterMimic load)
        - ``height``: scalar human height (meters) for robot / human scaling

    Args:
        file_path: Path to ``.npz``.

    Returns:
        human_joints, object_poses, height
    """
    data = np.load(file_path, allow_pickle=True)
    required = ("global_joint_positions", "object_poses", "height")
    missing = [k for k in required if k not in data.files]
    if missing:
        raise KeyError(f"{file_path} missing keys {missing}; have {sorted(data.files)}")

    human_joints = np.asarray(data["global_joint_positions"], dtype=np.float64)
    object_poses = np.asarray(data["object_poses"], dtype=np.float64)
    height = float(np.asarray(data["height"]).reshape(-1)[0])

    if human_joints.ndim != 3 or human_joints.shape[1] != 52 or human_joints.shape[2] != 3:
        raise ValueError(
            f"global_joint_positions must have shape (T, 52, 3), got {human_joints.shape} in {file_path}"
        )
    if object_poses.ndim != 2 or object_poses.shape[1] != 7:
        raise ValueError(f"object_poses must have shape (T, 7), got {object_poses.shape} in {file_path}")
    if not np.isfinite(height) or height <= 0:
        raise ValueError(f"height must be a positive finite scalar, got {height!r} in {file_path}")

    return human_joints, object_poses, height


def is_packaged_object_interaction_npz(path: str | Path) -> bool:
    """Return True if ``path`` is a numpy zip with Holosoma two-foot / packaged object-interaction keys."""
    try:
        data = np.load(str(path), allow_pickle=True)
        return {"global_joint_positions", "object_poses", "height"}.issubset(set(data.files))
    except OSError:
        return False
...
def transform_from_human_to_world(human_initial_root, object_initial_pose, local_translation):
    ...
    Args:
        ...
        object_initial_pose (np.ndarray): Object poses with shape (7). Call sites in
            ``robot_retarget`` pass ``[qw, qx, qy, qz, x, y, z]`` before MuJoCo reordering;
            horizontal object position is taken from the last three components' x,y (indices 4:6).
    ...
    """
    human_initial_root = np.asarray(human_initial_root, dtype=np.float64).reshape(3)
    object_initial_pose = np.asarray(object_initial_pose, dtype=np.float64).reshape(7)
    local_translation = np.asarray(local_translation, dtype=np.float64).reshape(3)

    if not np.all(np.isfinite(human_initial_root)) or not np.all(np.isfinite(object_initial_pose)):
        rotation_matrix = np.eye(3, dtype=np.float64)
        quat = R.from_matrix(rotation_matrix).as_quat(scalar_first=True)
        return rotation_matrix @ local_translation, quat

    # Translation x,y for [qw,qx,qy,qz,x,y,z] layout used before ``convert_object_poses_to_mujoco_order``.
    object_xy = object_initial_pose[4:6]
    human_to_object_2d = object_xy - human_initial_root[:2]
    dist = float(np.linalg.norm(human_to_object_2d))
    if dist < 1e-8:
        human_to_object_2d = np.array([1.0, 0.0], dtype=np.float64)
    else:
        human_to_object_2d = human_to_object_2d / dist

    x_axis_2d = human_to_object_2d
    ...
    # QR cleanup avoids NaN / ill-conditioned matrices when axes are nearly degenerate.
    q_mat, _ = np.linalg.qr(rotation_matrix)
    if np.linalg.det(q_mat) < 0:
        q_mat[:, -1] *= -1.0
    quat = R.from_matrix(q_mat).as_quat(scalar_first=True)
    return q_mat @ local_translation, quat
...
def _interp1d_fill_nan(y: np.ndarray) -> np.ndarray:
    """1D array: replace NaNs by linear interpolation in index; all-NaN becomes zeros."""
...
def interpolate_nans_xyz_joints(human_joints: np.ndarray) -> np.ndarray:
    """Linearly interpolate NaNs along time for each joint coordinate, shape (T, J, 3)."""
    x = np.asarray(human_joints, dtype=np.float64).copy()
    if x.ndim != 3 or x.shape[2] != 3:
        raise ValueError(f"human_joints must be (T, J, 3), got {x.shape}")
    _, j, _ = x.shape
    for ji in range(j):
        for k in range(3):
            x[:, ji, k] = _interp1d_fill_nan(x[:, ji, k])
    return x


def sanitize_object_poses_xyzw(object_poses: np.ndarray) -> np.ndarray:
    """Interpolate NaN object translations; invalid quaternions become identity (then normalized)."""
    op = np.asarray(object_poses, dtype=np.float64).copy()
    if op.ndim != 2 or op.shape[1] != 7:
        raise ValueError(f"object_poses must be (T, 7), got {op.shape}")
    t = op.shape[0]
    for k in range(4, 7):
        op[:, k] = _interp1d_fill_nan(op[:, k])
    ...
    for i in range(t):
        ...
        if (not np.all(np.isfinite(q))) or float(np.linalg.norm(q)) < 1e-12:
            ...
            op[i, :4] = q / np.linalg.norm(q)
    return op


def sanitize_object_interaction_motion(
    human_joints: np.ndarray, object_poses: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    """Remove NaNs / inf so interaction-mesh Delaunay and transforms stay finite."""
    hj = interpolate_nans_xyz_joints(human_joints)
    op = sanitize_object_poses_xyzw(object_poses)
    return hj, op
...
def create_interaction_mesh(vertices: np.ndarray):
    ...
    if not np.all(np.isfinite(v)):
        raise ValueError(
            "create_interaction_mesh: non-finite vertices (check mocap gaps / exporter). "
            f"bad_count={int(np.size(v) - np.sum(np.isfinite(v)))}"
        )
    tri = Delaunay(v)
    return v, tri.simplices
