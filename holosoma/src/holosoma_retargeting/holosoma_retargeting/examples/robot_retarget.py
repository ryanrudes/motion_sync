...
def load_motion_data(
    ...
    if task_type == "robot_only":
        ...
        pt_path = data_path / f"{task_name}.pt"
        if npz_path.exists():
            human_joints, object_poses, height = load_object_interaction_npz(str(npz_path))
            ...
            raise FileNotFoundError(
                f"Object-interaction motion not found: expected {npz_path} or {pt_path}"
            )
...
def main(cfg: RetargetingConfig) -> None:
    ...
    if task_type in {"object_interaction", "climbing"}:
        ...
        if n_bad:
            logger.warning(
                "Sanitizing %d non-finite motion samples (time interpolation, quaternion clamp)",
                n_bad,
            )
