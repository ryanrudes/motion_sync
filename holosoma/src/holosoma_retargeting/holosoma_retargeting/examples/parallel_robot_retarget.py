...
from holosoma_retargeting.src.utils import (  # type: ignore[import-not-found]  # noqa: E402
    ...
    is_packaged_object_interaction_npz,
    preprocess_motion_data,
    sanitize_object_interaction_motion,
)
...
def find_files(data_dir: Path, data_format: str, object_name: str | None = None, task_type: str | None = None):
    ...
    Args:
        ...
        task_type: When ``object_interaction``, ``smplh`` also picks packaged ``.npz`` motion files
    ...
    if data_format == "smplh":
        ...
        if object_name:
            files_pt = [str(p) for p in data_dir.glob(f"*{object_name}*.pt")]
        else:
            files_pt = [str(p) for p in data_dir.glob("*.pt")]
        files_npz: list[str] = []
        if task_type == "object_interaction":
            for p in data_dir.glob("*.npz"):
                if is_packaged_object_interaction_npz(p):
                    files_npz.append(str(p))
        return sorted(set(files_pt + files_npz))
...
def process_single_task(args):
    ...
    for k, aug_config in enumerate(augmentations):
        ...
        if task_type in {"object_interaction", "climbing"}:
            n_bad = int(np.sum(~np.isfinite(human_joints)) + np.sum(~np.isfinite(object_poses)))
            if n_bad:
                print(f"  Sanitizing {n_bad} non-finite motion samples (interp / quat clamp)")
            human_joints, object_poses = sanitize_object_interaction_motion(human_joints, object_poses)
