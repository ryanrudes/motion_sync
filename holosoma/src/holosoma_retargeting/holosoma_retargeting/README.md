...
### Two-foot skateboard (packaged NPZ)

Object interaction can load a **packaged** `.npz` (same keys as the `twofoot_retargeting` exporter) instead of InterMimic `.pt`:

- `global_joint_positions`: `(T, 52, 3)` in Holosoma `smplh` joint order
- `object_poses`: `(T, 7)` as `[qw, qx, qy, qz, x, y, z]`
- `height`: scalar human height (meters)

Skateboard collision/visual uses `--task-config.object-name skateboard` (see `models/skateboard/` and `models/g1/g1_29dof_w_skateboard.xml`).

```bash
# From this repo (after `uv run retargeting holosoma object-npz ...` writes demo.npz into holosoma_data/):
cd holosoma/src/holosoma_retargeting
python examples/robot_retarget.py \
  --data_path /path/to/dir_with_npz \
...
If both `demo.npz` and `demo.pt` exist, **`.npz` is preferred**.
...
# Object interaction (OMOMO or packaged two-foot `.npz` in the same folder)
